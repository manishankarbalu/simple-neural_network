...
test_label=[]
for i in range(100):
    test_record = [int(round(random.random())),int(round(random.random())),int(round(random.random()))]
    test_data.append(test_record)
    test_label.append([(test_record[0]^test_record[1]) * test_record[2]])
...
output_data = []
layer1 = nonlin(np.dot(test_data[80:],syn0))
layer2 = nonlin(np.dot(layer1,syn1))
for i,j in zip(layer2,test_label[80:]):
    print i[0],' : ',j[0]
